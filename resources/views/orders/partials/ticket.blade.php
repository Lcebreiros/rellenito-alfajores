{{-- resources/views/orders/partials/ticket.blade.php --}}

@php
  // Mostrar controles (Volver / Descargar) salvo que controls=0|false|off|no
  $controlsParam = strtolower((string)($controlsParam ?? request()->query('controls', '1')));
  $withControls = isset($withControls)
    ? (bool) $withControls
    : !in_array($controlsParam, ['0','false','off','no'], true);
  ...
  // Totales (reutilizable fuera de Livewire)
  $subtotalValue = isset($subtotal)
    ? (float) $subtotal
    : round((float) ($order->items?->sum(function ($i) {
        return !is_null($i->subtotal)
            ? (float) $i->subtotal
            : (float) $i->quantity * (float) $i->unit_price;
      }) ?? 0), 2);

  $discountValue = isset($discount)
    ? (float) $discount
    : round((float) ($order->discount ?? 0), 2);

  $taxValue = isset($tax)
    ? (float) $tax
    : round($subtotalValue * ((float) ($order->tax_rate ?? 0) / 100), 2);

  $totalValue = isset($total)
    ? (float) $total
    : (!is_null($order->total)
      ? (float) $order->total
      : round($subtotalValue + $taxValue - $discountValue, 2));

  $qrValue = $qr ?? request('qr');
  $isPdf = (bool) ($isPdf ?? false);

  $paymentValue = $paymentMethod ?? $order->payment_method ?? null;
  if ($paymentValue instanceof \UnitEnum) {
      $paymentValue = method_exists($paymentValue, 'value') ? $paymentValue->value : $paymentValue->name;
  }
@endphp
...
<div class="wrap">
  ...
  <div id="ticket" class="card ticket">
    ...
    @if($qrValue)
      <div class="qr">
        @php
          // Para PDFs, usar el QR generado localmente en base64 (evita timeouts)
          // Para HTML, usar el servicio externo (funciona bien en navegadores)
