...
@section('header')
  <h1 class="text-2xl font-semibold text-neutral-900 dark:text-neutral-100">{{ __('company.create_user_title') }}</h1>
@endsection
...
<div class="max-w-2xl mx-auto p-6">
  ...
  <div class="bg-white dark:bg-neutral-900 rounded-lg shadow border border-gray-200 dark:border-neutral-800 p-6">
    <form method="POST" action="{{ route('branch.users.store') }}">
      ...
      @if(isset($branches))
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-1">{{ __('company.field_branch') }}</label>
          <select name="branch_id" class="w-full px-3 py-2 border border-gray-300 dark:border-neutral-800 bg-white dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500" required>
            @if(auth()->user()?->isCompany())
              <option value="company" @selected(request('branch_id')==='company')>{{ __('company.company_no_branch') }}</option>
            @endif
      ...
      <div class="mt-6 flex items-center gap-3">
        ...
        <a href="{{ url()->previous() }}" class="px-4 py-2 border border-gray-300 dark:border-neutral-800 text-gray-700 dark:text-neutral-200 rounded-md hover:bg-gray-50 dark:hover:bg-neutral-800">{{ __('company.cancel_btn') }}</a>
      </div>
    </form>
  </div>
</div>
