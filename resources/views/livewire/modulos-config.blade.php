...
<div class="rounded-2xl border border-neutral-200 bg-white p-6 shadow
  ...
  <div class="flex items-center gap-3 mb-6">
    ...
    <div>
      <h2 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{{ __('settings.modules_title') }}</h2>
      <p class="text-xs text-neutral-500 dark:text-neutral-400">{{ __('settings.modules_subtitle') }}</p>
    </div>
  </div>
  ...
  <div class="space-y-4">
        ...
        <button type="button"
          ...
          <div class="flex flex-col items-center gap-2 text-center">
            ...
            <span class="text-xs text-neutral-600 dark:text-neutral-400">
              @if(in_array($key, $modulosActivos))
                {{ __('settings.module_visible') }}
              @else
                {{ __('settings.module_hidden') }}
              @endif
            </span>
          </div>
        </button>
    ...
    <div class="flex items-center justify-between pt-4 border-t border-neutral-200 dark:border-neutral-700">
      <div class="text-sm text-neutral-600 dark:text-neutral-400">
        <span class="font-semibold">{{ count($modulosActivos) }}</span> {{ __('settings.of_label') }} {{ count($availableModules) }} {{ __('settings.modules_selected_suffix') }}
      </div>
