@if($compact)
{{-- ======== COMPACT MODE: pill + dropdown ======== --}}
<div x-data="{ open: false }" class="relative" x-on:click.outside="open = false">

    {{-- Pill trigger --}}
    <button
        @click="open = !open"
        class="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-xl border text-xs font-semibold shadow-sm transition-colors
               {{ $session?->isOpen()
                   ? 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 dark:border-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400'
                   : 'border-neutral-200 bg-white text-neutral-600 hover:bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-800/60 dark:text-neutral-400' }}"
    >
        @if($session?->isOpen())
        ...
        @if($session?->isOpen())
            <span class="font-mono tabular-nums">${{ number_format($balance, 2, ',', '.') }}</span>
        @else
            <span>{{ __('cash.nav_label') }}</span>
        @endif
    ...
    <div
        ...
        class="absolute left-0 top-full mt-2 z-50 w-72 rounded-xl border border-neutral-200 dark:border-neutral-700
    ...
    >
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-neutral-200 dark:border-neutral-700">
            ...
            @if($session?->isOpen())
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700
                             dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-semibold">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    {{ __('cash.open') }}
                </span>
            @else
                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-neutral-100 text-neutral-500
                             dark:bg-neutral-800 dark:text-neutral-400 text-xs font-semibold">
                    {{ __('cash.closed') }}
                </span>
            @endif
        </div>
            ...
            <div class="p-4">
                @if(!$showOpenForm)
                    <button wire:click="$set('showOpenForm', true)"
                            class="w-full flex items-center justify-center gap-2 rounded-lg bg-emerald-600
                                   hover:bg-emerald-700 text-white px-4 py-2.5 text-sm font-semibold transition">
        ...
        @else
            ...
            @if(!$showMovementForm && !$showCloseForm)
                <div class="px-4 pb-3 flex gap-2">
                    <button wire:click="$set('movementType', 'ingreso'); $set('showMovementForm', true)"
                            class="flex-1 flex items-center justify-center gap-1 rounded-lg bg-blue-600 hover:bg-blue-700
                                   text-white px-3 py-2 text-sm font-semibold transition">
                        <span class="text-lg leading-none">+</span> {{ __('cash.income') }}
                    </button>
                    <button wire:click="$set('movementType', 'egreso'); $set('showMovementForm', true)"
                            class="flex-1 flex items-center justify-center gap-1 rounded-lg bg-orange-500 hover:bg-orange-600
                                   text-white px-3 py-2 text-sm font-semibold transition">
                ...
                <div class="px-4 pb-3 space-y-2 border-t border-neutral-200 dark:border-neutral-700 pt-3">
                    ...
                    <div>
                        ...
                        <input type="number" wire:model="closingAmount" min="0" step="0.01"
                               class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800
                                      px-3 py-2 text-sm text-right font-mono mt-1">
                    ...
                    <textarea wire:model="closingNote" rows="2" placeholder="{{ __('cash.closing_note_ph') }}"
                              class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-800
                                     px-3 py-2 text-sm resize-none"></textarea>
                    <div class="flex gap-2">
            ...
            @if(!$showMovementForm && !$showCloseForm && $movements->count())
                <div class="border-t border-neutral-200 dark:border-neutral-700">
                    ...
                    <div class="divide-y divide-neutral-100 dark:divide-neutral-800 max-h-40 overflow-y-auto">
                    ...
                    <div class="px-4 py-2 text-center border-t border-neutral-100 dark:border-neutral-800">
