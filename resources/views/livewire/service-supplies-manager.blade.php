<div class="rounded-lg border border-neutral-200 bg-white dark:border-neutral-800 dark:bg-neutral-900 p-6">
  <div class="mb-4">
    <h3 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100">{{ __('services.smgr_title') }}</h3>
    <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
      {{ __('services.smgr_subtitle') }}
    </p>
  </div>
  ...
  <form wire:submit.prevent="addSupply" class="mb-6 p-4 rounded-lg bg-neutral-50 dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700">
    <h4 class="text-sm font-medium text-neutral-900 dark:text-neutral-100 mb-3">{{ __('expenses.smgr_add_title') }}</h4>

    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div>
        <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
          {{ __('expenses.smgr_supply_label') }} <span class="text-rose-500">*</span>
        </label>
        <select wire:model="supply_id" class="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-900">
          <option value="">{{ __('expenses.smgr_select_ph') }}</option>
      ...
      <div>
        <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
          {{ __('expenses.smgr_qty_label') }} <span class="text-rose-500">*</span>
        </label>
      ...
      <div>
        <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">
          {{ __('expenses.smgr_unit_label') }} <span class="text-rose-500">*</span>
        </label>
        <select wire:model="unit" class="w-full rounded-lg border border-neutral-300 px-3 py-2 text-sm dark:border-neutral-700 dark:bg-neutral-900">
          <option value="g">{{ __('expenses.smgr_unit_g') }}</option>
          <option value="kg">{{ __('expenses.smgr_unit_kg') }}</option>
          <option value="ml">{{ __('expenses.smgr_unit_ml') }}</option>
          <option value="l">{{ __('expenses.smgr_unit_l') }}</option>
          <option value="cm3">{{ __('expenses.smgr_unit_cm3') }}</option>
          <option value="u">{{ __('expenses.smgr_unit_u') }}</option>
        </select>
    ...
    <div class="mt-4 flex justify-end">
      <button type="submit"
        ...
        {{ __('expenses.smgr_add_btn') }}
      </button>
    </div>
  </form>

  <!-- Lista de insumos asignados -->
  @if(empty($serviceSupplies))
    <div class="text-center py-8 text-neutral-500 dark:text-neutral-400">
      <svg class="mx-auto h-12 w-12 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
      </svg>
      <p class="mt-2 text-sm">{{ __('services.smgr_empty') }}</p>
    </div>
  @else
        ...
        <thead class="bg-neutral-50 dark:bg-neutral-800/50">
          <tr>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ __('expenses.smgr_col_supply') }}
            </th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ __('expenses.smgr_col_qty') }}
            </th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ __('expenses.smgr_col_unit') }}
            </th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ __('expenses.smgr_col_waste') }}
            </th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-neutral-500 dark:text-neutral-400 uppercase tracking-wider">
              {{ __('expenses.smgr_col_stock') }}
            </th>
          ...
          @foreach($serviceSupplies as $serviceSupply)
            <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition">
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-neutral-900 dark:text-neutral-100">
                {{ $serviceSupply['supply']['name'] ?? 'N/A' }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400 tabular-nums">
                {{ number_format($serviceSupply['qty'], 3, ',', '.') }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400">
                {{ strtoupper($serviceSupply['unit']) }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400 tabular-nums">
                {{ number_format($serviceSupply['waste_pct'], 2, ',', '.') }}%
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-neutral-600 dark:text-neutral-400 tabular-nums">
                {{ $serviceSupply['supply']['formatted_stock'] ?? number_format($serviceSupply['supply']['stock_base_qty'] ?? 0, 0, ',', '.') }} {{ strtoupper($serviceSupply['supply']['base_unit'] ?? '') }}
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button wire:click="removeSupply({{ $serviceSupply['id'] }})"
                        wire:confirm="{{ __('services.smgr_confirm_remove') }}"
