  ...
  <div class="bg-white dark:bg-neutral-950 rounded-3xl shadow-xl shadow-slate-200/20 dark:shadow-black/40 ring-1 ring-slate-200/50 dark:ring-neutral-800/60 overflow-hidden backdrop-blur-sm transition-all duration-200">
    <div class="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50/50 dark:from-blue-900/80 dark:to-indigo-900/40 border-b border-slate-200/60 dark:border-neutral-800/60">
      <h2 class="text-lg font-semibold text-slate-900 dark:text-neutral-50 flex items-center gap-3">
        ...
        {{ __('calculator.add_supply_title') }}
        <span class="ml-auto text-sm font-medium bg-gradient-to-r from-slate-600 to-slate-700 dark:from-neutral-300 dark:to-neutral-400 bg-clip-text text-transparent">
          {{ __('calculator.total_label') }} {{ number_format($this->count ?? 0, 0, ',', '.') }}
        </span>
      </h2>
    </div>
    ...
    <form wire:submit.prevent="quickStore" class="p-6 space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="space-y-2">
          <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300 mb-2">{{ __('calculator.field_name') }}</label>
        ...
        <div class="space-y-2">
          <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300 mb-2">{{ __('calculator.field_qty_purchased') }}</label>
        ...
        <div class="space-y-2">
          <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300 mb-2">{{ __('calculator.field_unit') }}</label>
          <select wire:model.defer="unit" required
                  class="w-full rounded-xl border border-slate-300/70 dark:border-neutral-700 bg-white dark:bg-neutral-900
                         px-4 py-3 text-sm text-slate-900 dark:text-neutral-100
                         focus:outline-none focus:ring-2 focus:ring-indigo-500/70 focus:border-indigo-500/70
                         transition-all duration-200 hover:border-slate-400 dark:hover:border-neutral-600">
            <optgroup label="{{ __('calculator.unit_group_mass') }}">
              <option value="g">g</option>
              <option value="kg">kg</option>
            </optgroup>
            <optgroup label="{{ __('calculator.unit_group_volume') }}">
              <option value="ml">ml</option>
              <option value="l">l</option>
              <option value="cm3">cm3</option>
            </optgroup>
            <optgroup label="{{ __('calculator.unit_group_unit') }}">
        ...
        <div class="space-y-2">
          <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300 mb-2">{{ __('calculator.field_total_price') }}</label>
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-slate-50 to-slate-100/50 dark:from-neutral-900 dark:to-neutral-900/80 border-b border-slate-200/60 dark:border-neutral-800/60">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h3 class="text-lg font-semibold text-slate-900 dark:text-neutral-50 flex items-center gap-3">
          ...
          {{ __('calculator.supplies_title') }}
        </h3>
        <input wire:model.debounce.400ms="search" placeholder="{{ __('calculator.search_ph') }}"
      ...
      <div class="space-y-4">
                ...
                <div class="space-y-1">
                  <div class="flex items-center gap-4 text-sm">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-slate-600 dark:text-neutral-400">{{ __('calculator.stock_label') }}</span>
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-slate-600 dark:text-neutral-400">{{ __('calculator.price_base_label') }}</span>
                      <span class="font-medium text-slate-900 dark:text-neutral-100">
                  ...
                  @if($last)
                    <div class="px-3 py-1 rounded-lg bg-slate-50 dark:bg-neutral-800/50 border border-slate-200/60 dark:border-neutral-800/60">
                      <p class="text-xs text-slate-500 dark:text-neutral-400">
                        <span class="font-medium">{{ __('calculator.last_purchase_label') }}</span>
                        {{ rtrim(rtrim(number_format((float)$last->qty, 6, ',', '.'), '0'), ',') }} {{ $last->unit }}
                        — {{ __('calculator.total_label') }} <span class="font-medium">${{ number_format((float)$last->total_cost, 2, ',', '.') }}</span>
                      </p>
                    </div>
                  @else
                    <div class="px-3 py-1 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200/60 dark:border-amber-800/40">
                      <p class="text-xs text-amber-600 dark:text-amber-400">{{ __('calculator.no_purchases') }}</p>
                    </div>
                  @endif
                </div>
              </div>
              ...
              <div class="mt-4 p-4 rounded-2xl bg-slate-50 dark:bg-neutral-900/50 border border-slate-200/60 dark:border-neutral-800/60">
                <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
                  ...
                  <div class="sm:col-span-3 space-y-2">
                    <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300">{{ __('calculator.field_name') }}</label>
                  ...
                  @if($editingPurchaseId)
                    <div class="sm:col-span-2 space-y-2">
                      <label class="block text-xs font-medium text-slate-600 dark:text-neutral-300">{{ __('calculator.field_last_price') }}</label>
        ...
        @empty
          <div class="py-12 text-center">
            ...
            <h3 class="text-sm font-medium text-slate-900 dark:text-neutral-100 mb-1">{{ __('calculator.supplies_empty_title') }}</h3>
            <p class="text-xs text-slate-500 dark:text-neutral-400">{{ __('calculator.supplies_empty_hint') }}</p>
          </div>
        @endforelse
      </div>
    </div>
