...
@section('header')
  <h1 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('support.page_title') }}</h1>
@endsection
...
@section('header_actions')
  <a href="#new" onclick="document.getElementById('newTicket').classList.toggle('hidden')"
    ...
    <i class="fas fa-plus"></i> {{ __('support.new_ticket_btn') }}
  </a>
...
<div class="max-w-5xl mx-auto px-3 sm:px-6 space-y-6">
  ...
  <div id="newTicket" class="hidden bg-white dark:bg-neutral-900 rounded-xl shadow-sm border border-neutral-100 dark:border-neutral-800 p-4">
    <form method="POST" action="{{ route('support.store') }}" class="space-y-3">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label class="block text-sm font-medium mb-1">{{ __('support.type_label') }}</label>
        ...
        <div>
          <label class="block text-sm font-medium mb-1">{{ __('support.subject_label') }}</label>
          <input name="subject" class="w-full rounded-lg border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2.5" placeholder="{{ __('support.subject_placeholder') }}">
        </div>
      </div>
      <div>
        <label class="block text-sm font-medium mb-1">{{ __('support.message_label') }}</label>
        <textarea name="message" rows="4" required class="w-full rounded-lg border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2.5" placeholder="{{ __('support.message_placeholder') }}"></textarea>
      </div>
      <div class="flex justify-end gap-2">
        <button type="button" onclick="document.getElementById('newTicket').classList.add('hidden')" class="px-4 py-2 rounded-lg border border-neutral-300 dark:border-neutral-700">{{ __('support.cancel_btn') }}</button>
        <button class="px-4 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-700">{{ __('support.send_btn') }}</button>
      </div>
    </form>
  </div>
  ...
  <div class="flex flex-wrap gap-2 text-sm items-center">
    ...
    <span class="text-neutral-500 dark:text-neutral-400">{{ __('support.status_label') }}</span>
    ...
    @endforeach
    <span class="ml-3 text-neutral-500 dark:text-neutral-400">{{ __('support.type_filter_label') }}</span>
    ...
    <div class="divide-y divide-neutral-200 dark:divide-neutral-800">
      @forelse($tickets as $t)
        <a href="{{ route('support.show', $t) }}" class="flex items-center justify-between p-4 hover:bg-neutral-50 dark:hover:bg-neutral-800/40">
          <div>
            <div class="font-semibold text-neutral-900 dark:text-neutral-100">{{ $t->subject ?: __('support.no_subject') }}</div>
      ...
      @empty
        <div class="p-8 text-center text-neutral-600 dark:text-neutral-300">{{ __('support.no_tickets') }}</div>
      @endforelse
    </div>
