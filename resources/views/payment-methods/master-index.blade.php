...
@section('header')
  <div class="flex items-center justify-between">
    <h1 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('payment_methods.master_title') }}</h1>
    <a href="{{ route('payment-methods.create') }}"
      ...
      <x-heroicon-o-plus class="w-4 h-4" /> {{ __('payment_methods.new_btn') }}
    </a>
  </div>
@endsection
  ...
  <div class="bg-white dark:bg-neutral-900 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-800 overflow-hidden">
    <div class="px-4 py-3 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
      <div class="text-sm text-neutral-600 dark:text-neutral-300">
        {{ __('payment_methods.master_subtitle') }}
      </div>
    </div>
    ...
    <div class="divide-y divide-neutral-200 dark:divide-neutral-800">
        ...
        <div class="p-4 flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              @endif
              @if($pm->is_active)
                <span class="inline-flex items-center px-1.5 py-0.5 text-[10px] rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">{{ __('payment_methods.status_active') }}</span>
              @else
                <span class="inline-flex items-center px-1.5 py-0.5 text-[10px] rounded-full bg-neutral-100 text-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">{{ __('payment_methods.status_inactive') }}</span>
              @endif
            </div>
            @if($pm->description)
          ...
          <div class="flex items-center gap-2">
            <form action="{{ route('payment-methods.toggle', $pm) }}" method="POST">
              ...
              <button type="submit"
                ...
                {{ $pm->is_active ? __('payment_methods.btn_deactivate') : __('payment_methods.btn_activate') }}
              </button>
            </form>
      ...
      @empty
        <div class="p-8 text-center">
          ...
          <div class="text-neutral-700 dark:text-neutral-300">{{ __('payment_methods.empty_master') }}</div>
          <div class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{{ __('payment_methods.empty_master_hint') }}</div>
        </div>
      @endforelse
    </div>
  </div>
