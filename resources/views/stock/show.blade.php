...
<div class="max-w-5xl mx-auto px-3 sm:px-6">
  ...
  <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-5 mb-6">
    <div class="flex flex-col sm:flex-row gap-4 sm:items-start sm:justify-between">
      ...
      <div class="text-right">
        ...
        <div class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ (int) $totalCompanyStock }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-5">
    ...
    @if($branchRows->isEmpty())
      <div class="text-sm text-neutral-500 dark:text-neutral-400">No hay sucursales o no hay stock distribuido en sucursales.</div>
    @else
      <div class="divide-y divide-neutral-100 dark:divide-neutral-800">
        @foreach($branchRows as $row)
          <div class="py-2 flex items-center justify-between">
            <div class="text-neutral-800 dark:text-neutral-200">{{ $row['name'] }}</div>
            <div class="font-semibold {{ $row['stock'] > 0 ? 'text-blue-600 dark:text-blue-300' : 'text-neutral-500 dark:text-neutral-400' }}">{{ (int) $row['stock'] }}</div>
          </div>
        @endforeach
      </div>
    @endif
