...
<div class="max-w-5xl mx-auto px-3 sm:px-6">
      ...
      <div>
        ...
        <div class="mt-1 text-sm text-neutral-600 dark:text-neutral-300">
          ...
          @if($creator['type']==='company')
            <span class="font-medium">{{ __('stock.type_company') }}</span> — {{ $creator['company_name'] ?? '—' }}
          @elseif($creator['type']==='branch')
            <span class="font-medium">{{ __('stock.type_branch') }}</span> — {{ $creator['branch_name'] ?? $creator['user_name'] ?? '—' }}
          @else
            <span class="font-medium">{{ __('stock.type_user') }}</span> — {{ $creator['user_name'] ?? '—' }}
          @endif
        </div>
      </div>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
    ...
    <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-700 p-5">
      <h3 class="text-base font-semibold text-neutral-900 dark:text-neutral-100 mb-3">{{ __('stock.by_branch') }}</h3>

      @if($branchRows->isEmpty())
        <div class="text-sm text-neutral-500 dark:text-neutral-400">{{ __('stock.no_branch_stock') }}</div>
      @else
