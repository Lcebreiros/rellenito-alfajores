...
<div class="container mx-auto px-4 py-6 max-w-7xl">
  ...
  <div class="mb-6 flex items-center justify-between">
    <div>
      <h1 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('discounts.title') }}</h1>
      <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
        {{ __('discounts.subtitle') }}
      </p>
    </div>
  ...
  @if($discounts->isEmpty())
    <div class="container-glass shadow-sm overflow-hidden">
      <div class="p-8 text-center">
        ...
        <h3 class="text-lg font-semibold text-neutral-900 dark:text-neutral-100 mb-2">{{ __('discounts.empty_title') }}</h3>
        <p class="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          {{ __('discounts.empty_body') }}
        </p>
        <a href="{{ route('discounts.create') }}"
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50">
              <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_name') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_type') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_value') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_partner') }}</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_validity') }}</th>
              <th class="px-4 py-3 text-center text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_status') }}</th>
              <th class="px-4 py-3 text-center text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase">{{ __('discounts.col_actions') }}</th>
            </tr>
          </thead>
                ...
                <td class="px-4 py-3 text-sm">
                  @if($discount->type === 'free_minutes')
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400 text-xs font-semibold">
                      ...
                      {{ __('discounts.type_free_minutes') }}
                    </span>
                  ...
                  @else
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-semibold">
                      ...
                      {{ __('discounts.type_fixed_amount') }}
                    </span>
                  @endif
                </td>
                ...
                <td class="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-400">
                  @if($discount->starts_at || $discount->ends_at)
                    @if($discount->starts_at)
                      <div>{{ __('discounts.from') }} {{ $discount->starts_at->format('d/m/Y') }}</div>
                    @endif
                    @if($discount->ends_at)
                      <div>{{ __('discounts.until') }} {{ $discount->ends_at->format('d/m/Y') }}</div>
                    @endif
                  @else
                    {{ __('discounts.permanent') }}
                  @endif
                </td>
                  ...
                  <div class="flex items-center justify-center gap-2">
                    <form method="POST" action="{{ route('discounts.toggle', $discount) }}" class="inline">
                      ...
                      <button type="submit"
                              ...
                              title="{{ $discount->is_active ? __('discounts.btn_deactivate') : __('discounts.btn_activate') }}">
                        @if($discount->is_active)
                    ...
                    <form method="POST" action="{{ route('discounts.destroy', $discount) }}" class="inline"
                          onsubmit="return confirm('{{ __('discounts.confirm_delete') }}');">
                      @csrf
                      @method('DELETE')
                      <button type="submit"
