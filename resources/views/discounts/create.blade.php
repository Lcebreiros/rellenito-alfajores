...
<div class="container mx-auto px-4 py-6 max-w-3xl">
  <div class="mb-6">
    <h1 class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('discounts.create_title') }}</h1>
    <p class="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
      {{ __('discounts.create_subtitle') }}
    </p>
  </div>
    ...
    <form method="POST" action="{{ route('discounts.store') }}">
      ...
      <div class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <select id="type" name="type" required
              ...
              <option value="free_minutes" {{ old('type') === 'free_minutes' ? 'selected' : '' }}>{{ __('discounts.type_free_minutes_option') }}</option>
              <option value="percentage" {{ old('type') === 'percentage' ? 'selected' : '' }}>{{ __('discounts.type_percentage') }}</option>
              <option value="fixed_amount" {{ old('type') === 'fixed_amount' ? 'selected' : '' }}>{{ __('discounts.type_fixed_amount') }}</option>
            </select>
            @error('type')
              <span class="text-red-600 text-xs mt-1">{{ $message }}</span>
            @enderror
        ...
        <div>
          ...
          <input type="text" id="partner" name="partner" value="{{ old('partner') }}"
                 class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm"
