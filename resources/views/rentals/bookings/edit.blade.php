...
@section('header')
  <div class="flex items-center gap-3">
    ...
    <div>
      <h1 class="text-xl sm:text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ __('rentals.edit_booking_title_prefix') }}{{ $booking->id }}</h1>
    </div>
  </div>
@endsection
...
<div class="max-w-xl mx-auto px-3 sm:px-6">
  ...
  <div class="container-glass shadow-sm overflow-hidden"
       x-data="{
         selectedSpaceId: '{{ $booking->rental_space_id }}',
         selectedOptionId: {{ $booking->rental_duration_option_id ?? 'null' }},
    ...
    <div class="px-5 py-4 border-b border-neutral-200 dark:border-neutral-700">
      <h2 class="text-base font-semibold text-neutral-900 dark:text-neutral-100">{{ __('rentals.edit_booking_title') }}</h2>
    </div>

    <form method="POST" action="{{ route('rentals.bookings.update', $booking) }}" class="p-5 space-y-4">
      @csrf
      @method('PUT')
      <input type="hidden" name="rental_duration_option_id" id="rental_duration_option_id"
             value="{{ old('rental_duration_option_id', $booking->rental_duration_option_id) }}">

      {{-- Espacio --}}
      <div>
        <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-200 mb-1">{{ __('rentals.field_space') }} *</label>
        <select name="rental_space_id" required x-model="selectedSpaceId"
          ...
          @foreach($spaces as $space)
            <option value="{{ $space->id }}" {{ old('rental_space_id', $booking->rental_space_id) == $space->id ? 'selected' : '' }}>
              {{ $space->name }}
            </option>
          @endforeach
        </select>
      ...
      <div>
        ...
        @if($booking->client_id)
          <select name="client_id"
            ...
            <option value="">{{ __('rentals.no_registered_client') }}</option>
      ...
      <div>
        ...
        <textarea name="notes" rows="2"
                  ...
                  placeholder="{{ __('rentals.notes_ph_short') }}">{{ old('notes', $booking->notes) }}</textarea>
      </div>
